...
这是一个围绕**忆阻器储层计算 (Memristor Reservoir Computing, RC)** 展开的神经形态计算研究项目。项目包含三个逻辑模块：一个 MNIST 概念验证子系统、一个面向临床的癫痫检测主系统、以及一套传统深度学习基线对比系统。
...
2. **CHB-MIT 头皮 EEG 癫痫发作检测**（核心临床应用，追求 F1 > 0.9 的临床可用级别）
...
| 层级 | 技术方案 | 关键创新 |
|------|---------|---------|
| **输入编码** | 固定绝对阈值 + 不应期 (Refractory Period) | 摒弃窗口级归一化，保留癫痫与正常状态的**绝对能量差异** |
...
| **后处理** | 滑动窗口多数投票 (Window=5, ~6秒) | 利用癫痫发作的持续性，滤除孤立假阳性，Precision 从 0.52 → 0.96 |
...
项目经历了三次关键架构迭代才达到当前性能：

1. **脉冲编码重建**：从"自适应密度阈值" → "固定绝对阈值 + 不应期"（解决正常/癫痫脉冲形态不可分问题）
2. **解码器升级**：从"单层全连接" → "1D CNN"（解决高维特征空间高度纠缠问题）
3. **时序平滑引入**：从"孤立窗口预测" → "滑动窗口多数投票"（解决类别不平衡导致的 Precision 崩溃问题）

这些迭代使得系统从**完全无法区分 (F1 ≈ 0)** 进化到 **临床可用级别 (F1 > 0.9)**。
...
- **`eeg_rc_project/README.md`**：EEG 癫痫检测项目的详细说明与参数速查表
